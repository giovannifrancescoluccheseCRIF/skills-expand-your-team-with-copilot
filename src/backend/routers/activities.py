...
def get_activities(
    ...
    - difficulty_level: Filter by difficulty level ('Beginner', 'Intermediate', 'Advanced', 'All')
    """
    ...
    # "All" means activities with no difficulty_level set (for all levels)
    if difficulty_level:
        if difficulty_level == "All":
            # Only return activities without a difficulty_level field
            query["difficulty_level"] = {"$exists": False}
        else:
            # Return activities matching the specified difficulty level
